...
public class CurveField<F extends Field> extends AbstractFieldOver<F, CurveElement> {
    ...
    public String toString() {
        ...
        f.put("a", Base58.encode(a.toBytes()));
        f.put("b", Base58.encode(b.toBytes()));
        f.put("gen", Base58.encode(gen.toBytes()));
        f.put("genNoCofac", Base58.encode(genNoCofac.toBytes()));
        f.put("order", Base58.encode(order.toByteArray()));
        f.put("cofac", Base58.encode(cofac.toByteArray()));
        //f.put("quotientCmp", Base58.encode(quotientCmp.toByteArray()));
        return  f.toJSONString();
    }

    public void setFromString(String str) {
        JSONObject f = JSONObject.parseObject(str);
        a.setFromBytes(Base58.decode(f.getString("a")));
        b.setFromBytes(Base58.decode(f.getString("b")));
        gen.setFromBytes(Base58.decode(f.getString("gen")));
        genNoCofac.setFromBytes(Base58.decode(f.getString("genNoCofac")));
        genPow = gen.getElementPowPreProcessing();
        order = Base58.decodeToBigInteger(f.getString("order"));
        cofac = Base58.decodeToBigInteger(f.getString("cofac"));
        //quotientCmp = Base58.decodeToBigInteger(f.getString("quotientCmp"));
    }
