...
//	}
	...
	private void transferCkpState(RandomAccessFile ckp, SocketChannel sChannel) {
		try {
			...
			Logger.println("---Took " + (System.currentTimeMillis() - milliInit) + " milliseconds to transfer the checkpoint");
			fileChannel.close();
		} catch (Exception e) {
			e.printStackTrace();
			System.err
	...
	private CommandsInfo[] recoverLogState(RandomAccessFile log, int endOffset) {
		try {
			...
			boolean mayRead = true;
			Logger.println("filepointer: " + log.getFilePointer() + " loglength " + logLength + " endoffset " + endOffset);
			while (mayRead) {
				try {
