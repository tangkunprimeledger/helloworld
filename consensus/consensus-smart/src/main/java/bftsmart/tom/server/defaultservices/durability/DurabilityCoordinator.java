    ...
    @Override public int setState(ApplicationState recvState) {
        ...
        if (recvState instanceof CSTState) {
            ...
            bftsmart.tom.util.Logger.println(
                "(DurabilityCoordinator.setState) I'm going to update myself from CID " + lastCheckpointCID + " to CID "
                    + lastCID);
        ...
        if (log == null) {
            ...
            if (config.isToLog()) {
                ...
                if (storedState.getLastCID() > -1) {
                    Logger.println("LAST CID RECOVERED FROM LOG: " + storedState.getLastCID());
                    setState(storedState);
                    getStateManager().setLastCID(storedState.getLastCID());
                } else {
                    Logger.println("REPLICA IS IN INITIAL STATE");
                }
            }
