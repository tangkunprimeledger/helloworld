...
public class LCManager {
    ...
    public boolean sound(HashSet<CollectData> collects) {
        ...
        for (CollectData c : collects) { // organize all existing timestamps and values separately
            ...
            bftsmart.tom.util.Logger.println(
                "(LCManager.sound) Context for replica " + c.getPid() + ": CID[" + c.getCid() + "] WRITESET[" + c
                    .getWriteSet() + "] (VALTS,VAL)[" + c.getQuorumWrites() + "]");

            timestamps
                .add(c.getQuorumWrites().getTimestamp()); //store timestamp received from a Byzatine quorum of WRITES
        ...
        for (int r : timestamps) {
            for (byte[] v : values) {
                ...
                bftsmart.tom.util.Logger.println(
                    "(LCManager.sound) testing predicate BIND for timestamp/value pair (" + r + " , " + Arrays
                        .toString(v) + ")");
                if (binds(r, v, collects)) {

                    bftsmart.tom.util.Logger.println(
                        "(LCManager.sound) Predicate BIND is true for timestamp/value pair (" + r + " , " + Arrays
                            .toString(v) + ")");
                    bftsmart.tom.util.Logger.println(
        ...
        if (unbound) {
            ...
            bftsmart.tom.util.Logger
                .println("(LCManager.sound) Predicate SOUND is true for the for context collected from N-F replicas");
        }
    ...
    public byte[] getBindValue(HashSet<CollectData> collects) {
        ...
        for (CollectData c : collects) { // organize all existing timestamps and values separately
            ...
            timestamps
                .add(c.getQuorumWrites().getTimestamp()); //store timestamp received from a Byzantine quorum of writes
    ...
    public boolean quorumHighest(int timestamp, byte[] value, HashSet<CollectData> collects) {
        ...
        for (CollectData c : collects) {
            ...
            if (c.getQuorumWrites().getTimestamp() == timestamp && Arrays
                .equals(value, c.getQuorumWrites().getValue())) {
        ...
        if (appears)
            bftsmart.tom.util.Logger.println(
                "(LCManager.quorumHighest) timestamp/value pair (" + timestamp + " , " + Arrays.toString(value)
                    + ") appears in at least one replica context");

        int count = 0;
        for (CollectData c : collects) {

            //bftsmartcustom.tom.util.Logger.println("\t\t[QUORUM HIGHEST] ts' < ts : " + (c.getQuorumWrites().getTimestamp() < timestamp));
            //bftsmartcustom.tom.util.Logger.println("\t\t[QUORUM HIGHEST] ts' = ts && val' = val : " + (c.getQuorumWrites().getTimestamp() == timestamp && Arrays.equals(value, c.getQuorumWrites().getValue())));
        ...
        if (quorum)
            bftsmart.tom.util.Logger.println(
                "(LCManager.quorumHighest) timestamp/value pair (" + timestamp + " , " + Arrays.toString(value)
                    + ") has the highest timestamp among a " + (SVController.getStaticConf().isBFT() ? "Byzantine" :
                    "simple") + " quorum of replica contexts");
    ...
    public boolean certifiedValue(int timestamp, byte[] value, HashSet<CollectData> collects) {
        ...
        for (CollectData c : collects) {
            ...
            for (TimestampValuePair pv : c.getWriteSet()) {
                ...
                //                bftsmartcustom.tom.util.Logger.println("\t\t[CERTIFIED VALUE] " + pv.getTimestamp() + "  >= " + timestamp);
                //                bftsmartcustom.tom.util.Logger.println("\t\t[CERTIFIED VALUE] " + Arrays.toString(value) + "  == " + Arrays.toString(pv.getValue()));
        ...
        if (certified)
            bftsmart.tom.util.Logger.println(
                "(LCManager.certifiedValue) timestamp/value pair (" + timestamp + " , " + Arrays.toString(value)
    ...
    public boolean hasValidProof(CertifiedDecision cDec) {
        ...
        for (ConsensusMessage consMsg : ConsensusMessages) {
            ...
            ConsensusMessage cm =
                new ConsensusMessage(consMsg.getType(), consMsg.getNumber(), consMsg.getEpoch(), consMsg.getSender(),
                    consMsg.getValue());
            ...
            if (consMsg.getProof() instanceof HashMap) { // Certificate is made of MAC vector
                ...
                if (recvMAC != null && myMAC != null && Arrays.equals(recvMAC, myMAC) && Arrays
                    .equals(consMsg.getValue(), hashedValue) && consMsg.getNumber() == cDec.getCID() && !alreadyCounted
                    .contains(consMsg.getSender())) {
                ...
                if (TOMUtil.verifySignature(pubRSAKey, data, signature) && !alreadyCounted
                    .contains(consMsg.getSender())) {
