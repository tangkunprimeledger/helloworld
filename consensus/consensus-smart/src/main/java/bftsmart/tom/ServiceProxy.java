...
public class ServiceProxy extends TOMSender {
    ...
    public ServiceProxy(int processId, String configHome, Comparator<byte[]> replyComparator,
        ...
        extractor = (replyExtractor != null) ? replyExtractor : new Extractor() {
            ...
            @Override public TOMMessage extractResponse(TOMMessage[] replies, int sameContent, int lastReceived) {
            ...
            try {
                if (reqType == TOMMessageType.UNORDERED_HASHED_REQUEST) {
                    ...
                    if (!this.sm.tryAcquire(invokeTimeout, TimeUnit.MILLISECONDS)) {
                        ...
                        Logger.println(getProcessId() + " // " + reqId + " // TIMEOUT // ");
    ...
    @Override public void replyReceived(TOMMessage reply) {
        Logger.println(
            "Synchronously received reply from " + reply.getSender() + " with sequence number " + reply.getSequence());
        try {
            canReceiveLock.lock();
    ...
    private class HashResponseController {
        ...
        public TOMMessage getResponse(int pos, TOMMessage tomMessage) {
            ...
            Logger.println(
                "[" + this.getClass().getName() + "] hashReplies[" + pos + "]=" + Arrays.toString(hashReplies[pos]));
