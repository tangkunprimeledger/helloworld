    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i=0; i<warmup; i++) {
            ...
            if(id == 0) {
                ...
                //Logger.println(();
                Logger.println("Roundtrip "+((System.nanoTime()-time)/1000.0)+" us");
            } else {
        ...
        Logger.println("Beginning the real test with "+test+" roundtrips");
        ...
        Logger.println("Average time for "+test+" executions (-10%) = "+st.getAverage(true)/1000+ " us ");
        Logger.println("Standard desviation for "+test+" executions (-10%) = "+st.getDP(true)/1000 + " us ");
        Logger.println("Maximum time for "+test+" executions (-10%) = "+st.getMax(true)/1000+ " us ");
        Logger.println("Average time for "+test+" executions (all samples) = "+st.getAverage(false)/1000+ " us ");
        Logger.println("Standard desviation for "+test+" executions (all samples) = "+st.getDP(false)/1000 + " us ");
        Logger.println("Maximum time for "+test+" executions (all samples) = "+st.getMax(false)/1000+ " us ");
