...
public class StandardStateManager extends BaseStateManager {
    ...
    @Override protected void requestState() {
        ...
        SMMessage smsg =
            new StandardSMMessage(SVController.getStaticConf().getProcessId(), waitingCID, TOMUtil.SM_REQUEST, replica,
                null, null, -1, -1);
        ...
        TimerTask stateTask = new TimerTask() {
            public void run() {
                Logger.println("Timeout to retrieve state");
    ...
    @Override public void SMRequestDeliver(SMMessage msg, boolean isBFT) {
        if (SVController.getStaticConf().isStateTransferEnabled() && dt.getRecoverer() != null) {
            ...
            Logger.println("-- Should I send the state? " + sendState);

            ApplicationState thisState = dt.getRecoverer().getState(msg.getCID(), sendState);
            if (thisState == null) {
                Logger.println("-- For some reason, I am sending a void state");
                thisState = dt.getRecoverer().getState(-1, sendState);
            } else {
                Logger.println("-- Will I send the state? " + (thisState.getSerializedState() != null));
            }
            int[] targets = {msg.getSender()};
            SMMessage smsg =
                new StandardSMMessage(SVController.getStaticConf().getProcessId(), msg.getCID(), TOMUtil.SM_REPLY, -1,
                    thisState, SVController.getCurrentView(), tomLayer.getSynchronizer().getLCManager().getLastReg(),
                    tomLayer.execManager.getCurrentLeader());

            Logger.println("Sending state");
        ...
        if (SVController.getStaticConf().isStateTransferEnabled()) {
            if (waitingCID != -1 && msg.getCID() == waitingCID) {
                ...
                if (msg.getSender() == replica && msg.getState().getSerializedState() != null) {
                    Logger.println("Expected replica sent state. Setting it to state");
                    state = msg.getState();
                    if (stateTimer != null)
                        stateTimer.cancel();
                }
                ...
                Logger.println("Verifying more than F replies");
                if (enoughReplies()) {
                    Logger.println("More than F confirmed");
                    ApplicationState otherReplicaState = getOtherReplicaState();
                    Logger.println(
                        "State != null: " + (state != null) + ", recvState != null: " + (otherReplicaState != null));
                    int haveState = 0;
                    if (state != null) {
                        byte[] hash = null;
                    ...
                    if (otherReplicaState != null && haveState == 1 && currentRegency > -1 && currentLeader > -1
                        ...
                        Logger.println("Received state. Will install it");
                        ...
                        Logger.println(" -- State transfer not yet finished");
