...
public class DurableStateManager extends BaseStateManager {
    ...
    @Override public void init(TOMLayer tomLayer, DeliveryThread dt) {
    ...
    @Override protected void requestState() {
        ...
        Logger.println("(TOMLayer.requestState) I just sent a request to the other replicas for the state up to CID "
            + waitingCID);
    ...
    @Override public void SMRequestDeliver(SMMessage msg, boolean isBFT) {
        ...
        if (SVController.getStaticConf().isStateTransferEnabled() && dt.getRecoverer() != null) {
            ...
            Logger.println(
                "(TOMLayer.SMRequestDeliver) I received a state request for CID " + msg.getCID() + " from replica "
                    + msg.getSender());
            ...
            Logger.println("--- state asked");

            int[] targets = {msg.getSender()};
            InetSocketAddress address =
                SVController.getCurrentView().getAddress(SVController.getStaticConf().getProcessId());
            ...
            CSTSMMessage reply =
                new CSTSMMessage(myId, msg.getCID(), TOMUtil.SM_REPLY, cstConfig, null, SVController.getCurrentView(),
    ...
    @Override public void SMReplyDeliver(SMMessage msg, boolean isBFT) {
        lockTimer.lock();
        ...
        if (SVController.getStaticConf().isStateTransferEnabled()) {
            ...
            Logger.println(
                "(TOMLayer.SMReplyDeliver) I received a state reply for CID " + reply.getCID() + " from replica "
                    + reply.getSender());

            Logger.println("--- Received CID: " + reply.getCID() + ". Waiting " + waitingCID);
            if (waitingCID != -1 && reply.getCID() == waitingCID) {
                ...
                if (senderStates.size() == 3) {
                    ...
                    Logger.println("lowerLog ");
                    if (lowerLog != null)
                        Logger.println("lowerLog length:" + lowerLog.length);
                    Logger.println("upperLog ");
                    if (upperLog != null)
                        Logger.println("upperLog length" + upperLog.length);

                    boolean haveState = false;
                    byte[] lowerbytes = TOMUtil.getBytes(lowerLog);
                    Logger.println("Log lower bytes size: " + lowerbytes.length);
                    byte[] upperbytes = TOMUtil.getBytes(upperLog);
                    Logger.println("Log upper bytes size: " + upperbytes.length);
                    ...
                    if (Arrays.equals(stateCkp.getHashLogLower(), lowerLogHash))
                        ...
                        Logger.println("Lower log don't match");
                    // validate upper log
                    if (!haveState || !Arrays.equals(stateCkp.getHashLogUpper(), upperLogHash)) {
                        haveState = false;
                        Logger.println("Upper log don't match");
                    }

                    CSTState statePlusLower =
                        new CSTState(stateCkp.getSerializedState(), TOMUtil.getBytes(stateCkp.getSerializedState()),
                    ...
                    if (haveState) { // validate checkpoint
                        Logger.println("validating checkpoint!!!");
                        ...
                        if (!Arrays.equals(currentStateHash, stateUpper.getHashCheckpoint())) {
                            Logger.println("ckp hash don't match");
                            haveState = false;
                        }
                    }

                    Logger.println("-- current regency: " + currentRegency);
                    Logger.println("-- current leader: " + currentLeader);
                    Logger.println("-- current view: " + currentView);
                    if (currentRegency > -1 && currentLeader > -1 && currentView != null && haveState && (!isBFT
                        || /*currentProof != null ||*/ appStateOnly)) {
                        Logger.println("---- RECEIVED VALID STATE ----");
                        ...
                        //
                        //							Logger.println(("Installing proof for consensus " + waitingCID);
                        //
                        ...
                        //
                        //									Logger.println(("Strange... proof contains messages from more than just one epoch");
                        //									e = cons.getEpoch(cm.getEpoch(), true, SVController);
                        //								}
                        //								e.addToProof(cm);
                        //
                        ...
                        //
                        //								Logger.println(("Successfully installed proof for consensus " + waitingCID);
                        //
                        //							} else {
                        //								Logger.println(("Failed to install proof for consensus " + waitingCID);
                        //
                        //							}
                        //
                        //						}
                        ...
                        Logger.println("trying to acquire deliverlock");
                        dt.deliverLock();
                        Logger.println("acquired");
                        ...
                        if (SVController.getCurrentViewId() != currentView.getId()) {
                            Logger.println("Installing current view!");
                        ...
                        Logger.println("I updated the state!");
                        ...
                        Logger.println("---- DIDNT RECEIVE STATE ----");

                        Logger.println(
                            "(TOMLayer.SMReplyDeliver) I have more than " + (SVController.getCurrentViewN() / 2)
                        ...
                        Logger.println("---- RECEIVED INVALID STATE  ----");
