...
public final class Acceptor {
    ...
    private void executePropose(Epoch epoch, byte[] value) {
        ...
        if (epoch.propValue == null) { //only accept one propose per epoch
            ...
            Logger.println("(Acceptor.executePropose) I have written value " + Arrays.toString(epoch.propValueHash)
            ...
            if (epoch.deserializedPropValue != null && !epoch.isWriteSetted(me)) {
                ...
                if (controller.getStaticConf().isBFT()) {
                    ...
                    /**** LEADER CHANGE CODE! ******/
                    Logger.println(
                        "(Acceptor.executePropose) [CFT Mode] Setting consensus " + cid + " QuorumWrite tiemstamp to "
    ...
    private void computeWrite(int cid, Epoch epoch, byte[] value) {
        ...
        if (writeAccepted > controller.getQuorum() && Arrays.equals(value, epoch.propValueHash)) {
            ...
            if (!epoch.isAcceptSetted(me)) {
                ...
                /**** LEADER CHANGE CODE! ******/
                Logger.println("(Acceptor.computeWrite) Setting consensus " + cid + " QuorumWrite tiemstamp to " + epoch
                    .getConsensus().getEts() + " and value " + Arrays.toString(value));
                ...
                Logger.println(
                    "(Acceptor.computeWrite) Creating cryptographic proof for my ACCEPT message from consensus " + cid);
    ...
    private void computeAccept(int cid, Epoch epoch, byte[] value) {
        Logger.println(
            "(Acceptor.computeAccept) I have " + epoch.countAccept(value) + " ACCEPTs for " + cid + "," + epoch
                .getTimestamp());
