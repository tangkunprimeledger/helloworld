...
*/
package com.higgs.trust.consensus.bftsmart.communication.client.netty;

import com.higgs.trust.consensus.bftsmart.reconfiguration.ViewController;
...
public class NettyTOMMessageDecoder extends ByteToMessageDecoder {
    ...
    public NettyTOMMessageDecoder(boolean isClient, Map sessionTable, int macLength, ViewController controller, ReentrantReadWriteLock rl, int signatureLength, boolean useMAC) {
        ...
        this.useMAC = useMAC;
        com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("new NettyTOMMessageDecoder!!, isClient=" + isClient);
    }
        ...
        try {
            ...
            if (isClient) {
                ...
                if (sessionTable.containsKey(sm.getSender())) {
                    ...
                    com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("Creating MAC/public key stuff, first message from client" + sm.getSender());
                    com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("sessionTable size=" + sessionTable.size());
...
//                    logger.info("PUT INTO SESSIONTABLE - [client id]:"+sm.getSender()+" [channel]: "+cs.getChannel());
                    ...
                    com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("#active clients " + sessionTable.size());
                    rl.writeLock().unlock();
            ...
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("Impossible to decode message: "+
                    ex.getMessage());
            ex.printStackTrace();
        }
        return;
