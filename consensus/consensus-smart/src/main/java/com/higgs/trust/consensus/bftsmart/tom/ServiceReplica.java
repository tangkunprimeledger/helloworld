 ...
 * the License.
 */
package com.higgs.trust.consensus.bftsmart.tom;

import com.higgs.trust.consensus.bftsmart.communication.ServerCommunicationSystem;
import com.higgs.trust.consensus.bftsmart.consensus.messages.MessageFactory;
import com.higgs.trust.consensus.bftsmart.consensus.roles.Acceptor;
import com.higgs.trust.consensus.bftsmart.consensus.roles.Proposer;
import com.higgs.trust.consensus.bftsmart.reconfiguration.ReconfigureReply;
import com.higgs.trust.consensus.bftsmart.reconfiguration.ServerViewController;
import com.higgs.trust.consensus.bftsmart.reconfiguration.VMMessage;
import com.higgs.trust.consensus.bftsmart.tom.core.ExecutionManager;
import com.higgs.trust.consensus.bftsmart.tom.core.ReplyManager;
...
import com.higgs.trust.consensus.bftsmart.tom.leaderchange.CertifiedDecision;
import com.higgs.trust.consensus.bftsmart.tom.server.*;
import com.higgs.trust.consensus.bftsmart.tom.server.defaultservices.DefaultReplier;
import com.higgs.trust.consensus.bftsmart.tom.util.ShutdownHookThread;
    ...
    public void receiveMessages(int consId[], int regencies[], int leaders[], CertifiedDecision[] cDecs, TOMMessage[][] requests) {
        ...
        for (TOMMessage[] requestsFromConsensus : requests) {
            ...
            for (TOMMessage request : requestsFromConsensus) {
                ...
                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) Processing TOMMessage from client " + request.getSender() + " with sequence number " + request.getSequence() + " for session " + request.getSession() + " decided in consensus " + consId[consensusCount]);
                ...
                if (request.getViewID() == SVController.getCurrentViewId()) {
                    ...
                    if (request.getReqType() == TOMMessageType.ORDERED_REQUEST) {
                        ...
                        if (executor instanceof BatchExecutable) {
                            ...
                            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) Batching request from " + request.getSender());
                            ...
                            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) Delivering request from " + request.getSender() + " via FifoExecutable");
                            ...
                            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) sending reply to " + request.getSender());
                            replier.manageReply(request, msgCtx);
                        } else if (executor instanceof SingleExecutable) {

                            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) Delivering request from " + request.getSender() + " via SingleExecutable");
            ...
            if (noop && this.recoverer != null) {
                ...
                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) Delivering a no-op to the recoverer");
        ...
        if (executor instanceof BatchExecutable && numRequests > 0) {
            ...
            for (int index = 0; index < toBatch.size(); index++) {
                ...
                if (SVController.getStaticConf().getNumRepliers() > 0) {
                    com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) sending reply to " + request.getSender() + " with sequence number " + request.getSequence() + " and operation ID " + request.getOperationId() +" via ReplyManager");
                    repMan.send(request);
                } else {
                    com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(ServiceReplica.receiveMessages) sending reply to " + request.getSender() + " with sequence number " + request.getSequence() + " and operation ID " + request.getOperationId());
                    replier.manageReply(request, msgContexts[index]);
                    //cs.send(new int[]{request.getSender()}, request.reply);
                }
            }
            //DEBUG
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("BATCHEXECUTOR END");
        }
    }
