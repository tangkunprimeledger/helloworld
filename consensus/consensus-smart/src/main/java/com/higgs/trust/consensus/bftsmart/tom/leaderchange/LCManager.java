...
*/
package com.higgs.trust.consensus.bftsmart.tom.leaderchange;

import com.higgs.trust.consensus.bftsmart.communication.server.ServerConnection;
import com.higgs.trust.consensus.bftsmart.consensus.TimestampValuePair;
import com.higgs.trust.consensus.bftsmart.consensus.messages.ConsensusMessage;
import com.higgs.trust.consensus.bftsmart.reconfiguration.ServerViewController;
...
public class LCManager {
    ...
    public boolean sound(HashSet<CollectData> collects) {
        ...
        com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) I collected the context from " + collects.size() + " replicas");
        ...
        for (CollectData c : collects) { // organize all existing timestamps and values separately
            ...
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) Context for replica "+c.getPid()+": CID["+c.getCid()+"] WRITESET["+c.getWriteSet()+"] (VALTS,VAL)[" + c.getQuorumWrites() +"]");
        ...
        com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) number of timestamps: "+timestamps.size());
        com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) number of values: "+values.size());

        // after having organized all timestamps and values, properly apply the predicate
        for (int r : timestamps) {
            for (byte[] v : values) {

                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) testing predicate BIND for timestamp/value pair (" + r + " , " + Arrays.toString(v) + ")");
                if (binds(r, v, collects)) {

                    com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) Predicate BIND is true for timestamp/value pair (" + r + " , " + Arrays.toString(v) + ")");
                    com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) Predicate SOUND is true for the for context collected from N-F replicas");
                    return true;
                }
            }
        }

        com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) No timestamp/value pair passed on the BIND predicate");
        
        boolean unbound = unbound(collects);
        
        if (unbound) {
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) Predicate UNBOUND is true for N-F replicas");
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.sound) Predicate SOUND is true for the for context collected from N-F replicas");
        }
    ...
    public boolean binds(int timestamp, byte[] value, HashSet<CollectData> collects) {
        ...
        if (value == null || collects == null) {
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.binds) Received null objects, returning false");
            return false;
        }
        
        if (!(collects.size() >= (SVController.getCurrentViewN() - SVController.getCurrentViewF()))) {
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.binds) Less than N-F contexts collected from replicas, returning false");
            return false;
        }
    ...
    public boolean quorumHighest(int timestamp, byte[] value, HashSet<CollectData> collects) {
        ...
        if (appears) com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.quorumHighest) timestamp/value pair (" + timestamp + " , " + Arrays.toString(value) + ") appears in at least one replica context");
        ...
        if (quorum) com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.quorumHighest) timestamp/value pair (" + timestamp + " , " + Arrays.toString(value) +
...
//                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("\t\t[CERTIFIED VALUE] " + pv.getTimestamp() + "  >= " + timestamp);
//                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("\t\t[CERTIFIED VALUE] " + Arrays.toString(value) + "  == " + Arrays.toString(pv.getValue()));
        ...
        if (certified) com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.certifiedValue) timestamp/value pair (" + timestamp + " , " + Arrays.toString(value) +
    ...
    public boolean hasValidProof(CertifiedDecision cDec) {
        ...
        for (ConsensusMessage consMsg : ConsensusMessages) {
            ...
            if (consMsg.getProof() instanceof HashMap) { // Certificate is made of MAC vector
                ...
                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.hasValidProof) Proof made of MAC vector");
                ...
                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.hasValidProof) Proof made of Signatures");
                ...
                com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.hasValidProof) Proof is message is invalid");
            }
        }
        ...
        if (certificateLastView != -1 && pubRSAKey != null)
            com.higgs.trust.consensus.bftsmart.tom.util.Logger.println("(LCManager.hasValidProof) Computing certificate based on previous view");
