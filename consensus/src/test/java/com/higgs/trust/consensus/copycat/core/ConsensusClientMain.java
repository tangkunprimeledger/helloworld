...
@Slf4j public class ConsensusClientMain {
    ...
    private static void startServer(Address address, List<Address> clusterAddress) {
        ...
        Storage storage =
            Storage.builder().withStorageLevel(StorageLevel.DISK).withDirectory("D:/temp/copycat" + address.port())
                .withMinorCompactionInterval(Duration.ofMillis(1000)).withEntryBufferSize(1000).withCompactionThreads(1)
                .withMaxEntriesPerSegment(1000).withMajorCompactionInterval(Duration.ofMillis(2000))
