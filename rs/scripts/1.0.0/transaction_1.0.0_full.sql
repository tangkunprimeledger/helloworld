...
CREATE TABLE IF NOT EXISTS `core_transaction` (
	...
	`biz_model` TEXT DEFAULT NULL COMMENT 'the save data of the biz',
	...
	`signature` VARCHAR (255) NOT NULL COMMENT 'the signature for tx of the create rs',
	`status` VARCHAR (32) NOT NULL COMMENT 'the status of the row: 1.INIT 2.ALREADY_SEND 3.FAIL 4.SUCCESS',
	`create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'create time',
	`update_time` datetime NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'update time',
	PRIMARY KEY (`id`),
	UNIQUE KEY `uniq_tx_id` (`tx_id`)
) ENGINE = INNODB DEFAULT CHARSET = utf8mb4 COMMENT = 'the table of core transaction';
...
) ENGINE = INNODB DEFAULT CHARSET = utf8mb4 COMMENT = 'the table of core action';
