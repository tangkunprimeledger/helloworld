...
/////////////////////////////////////////////////
public class CustomerRetryableFeignLoadBalancer extends CustomerFeignLoadBalancer implements ServiceInstanceChooser {
	...
	public CustomerRetryableFeignLoadBalancer(ILoadBalancer lb, IClientConfig clientConfig,
	...
	public CustomerRetryableFeignLoadBalancer(ILoadBalancer lb, IClientConfig clientConfig, ServerIntrospector serverIntrospector,
		...
		return retryTemplate.execute(new RetryCallback<RibbonResponse, IOException>() {
			...
			public RibbonResponse doWithRetry(RetryContext retryContext) throws IOException {
				...
				if (retryPolicy.retryableStatusCode(response.status())) {
					byte[] byteArray = response.body() == null ? new byte[]{} : StreamUtils
                        .copyToByteArray(response.body().asInputStream());
					response.close();
					throw new RibbonResponseStatusCodeException(
                        CustomerRetryableFeignLoadBalancer.this.clientName, response,
	...
	public RequestSpecificRetryHandler getRequestSpecificRetryHandler(
			CustomerFeignLoadBalancer.RibbonRequest request, IClientConfig requestConfig) {
